...
def print_input_config(
        args: Any, *, instances: Sequence[Mapping[str, Any]] | None = None,
        instance_runtime_configs: Sequence[Mapping[str, Any]] | None = None,
        ) -> None:
    """Render the effective configuration block used at startup.

    ``argparse`` values are fallbacks for fields that may be overridden per
    instance in the cluster config.  When resolved instance configs are
    supplied, omit those potentially misleading fallback values and render
    the values that will actually be passed to each scheduler instead.
    """
    if (instances is None) != (instance_runtime_configs is None):
        raise ValueError(
            "instances and instance_runtime_configs must be supplied together"
        )
    if (instances is not None
            and len(instances) != len(instance_runtime_configs)):
        raise ValueError(
            "instances and instance_runtime_configs must have equal length"
        )
    show_effective_runtime = instance_runtime_configs is not None
    ...
    if show_effective_runtime:
        _console.print()
        _console.print(
            "[sim.tagline]Effective per-instance runtime[/]",
            justify="center",
        )
        _console.print()
        for index, (instance, runtime) in enumerate(zip(
                instances, instance_runtime_configs)):
            instance_id = instance.get("instance_id", index)
            role = instance.get("pd_type") or "colocated"
            threshold = runtime["long_prefill_token_threshold"]
            threshold_text = threshold if threshold else "disabled"
            details = (
                f"max_model_len={runtime['max_model_len']}, "
                f"max_num_seqs={_inf0(runtime['max_num_seqs'])}, "
                "max_num_batched_tokens="
                f"{_inf0(runtime['max_num_batched_tokens'])}, "
                f"long_prefill_token_threshold={threshold_text}, "
                f"block_size={runtime['block_size']}, "
                f"dtype={runtime['dtype']}, "
                f"kv_cache_dtype={runtime['kv_cache_dtype']}, "
                "chunked_prefill="
                f"{_yn(runtime['enable_chunked_prefill'])}, "
                f"prefix_caching={_yn(runtime['enable_prefix_caching'])}, "
                f"latency_model={_na(runtime['latency_model'])}, "
                f"latency_model_band={runtime['latency_model_band']}"
            )
            _console.print(
                f"  • [cyan]Instance {instance_id} ({role})[/cyan] : "
                f"{details}"
            )
    _console.rule(style="sim.rule")
