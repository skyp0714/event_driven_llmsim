...
_ASTRA_GBPS_TO_BYTES_PER_NS = (1 << 30) / 1_000_000_000
_LINK_BW_UNITS = {"astra_GBps", "decimal_GBps"}


def _link_bw_for_astra(link_bw, link_bw_unit=None):
    """Return bandwidth values in ASTRA's historical config units.

    ASTRA's analytical backend interprets one numeric ``GB/s`` unit as
    ``2**30 / 1e9`` bytes/ns. Existing cluster configs intentionally retain
    that historical interpretation. An explicit ``decimal_GBps`` annotation
    instead preserves the declared value as decimal bytes/ns by compensating
    for ASTRA's conversion when generating ``network.yml``.
    """
    if link_bw_unit is None or link_bw_unit == "astra_GBps":
        scale = 1.0
    elif link_bw_unit == "decimal_GBps":
        scale = 1.0 / _ASTRA_GBPS_TO_BYTES_PER_NS
    else:
        allowed = ", ".join(sorted(_LINK_BW_UNITS))
        raise ValueError(
            f"Unsupported link_bw_unit '{link_bw_unit}'; expected one of: "
            f"{allowed}"
        )
    ...
    def convert(value):
        if isinstance(value, bool) or not isinstance(value, (int, float)):
            raise TypeError(
                "link_bw must be a number or an array of numbers")
        value = float(value)
        if not math.isfinite(value) or value <= 0:
            raise ValueError("link_bw values must be finite and positive")
        return value * scale

    if isinstance(link_bw, (list, tuple)):
        return [convert(value) for value in link_bw]
    return convert(link_bw)


def _resolve_cluster_config_path(cluster_config_path):
    """Resolve a repo-relative config from the ASTRA-Sim working directory."""
    cluster_config_path = os.fspath(cluster_config_path)
    if os.path.isabs(cluster_config_path):
        return cluster_config_path
    return os.path.join("..", cluster_config_path)
...
def _compute_pd_endpoint_aliases(instances):
    """Map virtual PREFILL receiver ranks onto unambiguous decode ranks.

    PREFILL conversion emits receive-only Chakra graphs on a second, virtual
    rank set so those receives can run concurrently with decode workloads.
    The callback identity must remain virtual, but the analytical network can
    route its traffic through the physical decode endpoint.  This list maps
    every logical rank to the rank whose network resources it should use.

    A prefill instance is aliased only when its decode peer is unambiguous and
    has the same rank layout.  With multiple compatible decode replicas the
    router chooses the destination after prefill completes, so guessing here
    would charge contention to the wrong endpoint.
    """
    rank_starts = {}
    current_rank = 0
    for inst in instances:
        rank_starts[inst["instance_id"]] = current_rank
        effective_npus = inst["num_npus"]
        if inst.get("pd_type") == "prefill":
            effective_npus *= 2
        current_rank += effective_npus

    aliases = list(range(current_rank))
    mapped = {}
    unresolved = []
    decode_instances = [
        inst for inst in instances if inst.get("pd_type") == "decode"
    ]

    for prefill in instances:
        if prefill.get("pd_type") != "prefill":
            continue
        candidates = [
            decode for decode in decode_instances
            if decode.get("node_id") == prefill.get("node_id")
            and decode.get("model_name") == prefill.get("model_name")
            and decode.get("num_npus") == prefill.get("num_npus")
            and decode.get("tp_size") == prefill.get("tp_size")
            and decode.get("pp_size") == prefill.get("pp_size")
        ]
        if len(candidates) != 1:
            unresolved.append(prefill["instance_id"])
            continue

        decode = candidates[0]
        prefill_start = rank_starts[prefill["instance_id"]]
        decode_start = rank_starts[decode["instance_id"]]
        shadow_start = prefill_start + prefill["num_npus"]
        for rank_offset in range(prefill["num_npus"]):
            shadow_rank = shadow_start + rank_offset
            decode_rank = decode_start + rank_offset
            aliases[shadow_rank] = decode_rank
            mapped[shadow_rank] = decode_rank

    return aliases, mapped, unresolved
...
def build_cluster_config(
        ...
        hbf_num_cards=None, wakekv_hbf_num_cards=None):
    if (
        hbf_num_cards is not None
        and wakekv_hbf_num_cards is not None
        and hbf_num_cards != wakekv_hbf_num_cards
    ):
        raise ValueError(
            "hbf_num_cards and legacy wakekv_hbf_num_cards disagree")
    if hbf_num_cards is None:
        hbf_num_cards = wakekv_hbf_num_cards
    if hbf_num_cards is not None:
        if (isinstance(hbf_num_cards, bool)
                or not isinstance(hbf_num_cards, int)
                or hbf_num_cards <= 0):
            raise ValueError(
                "hbf_num_cards must be a positive integer")
    cluster_config_path = _resolve_cluster_config_path(cluster_config_path)
    ...
    memory_config = {}
    if hbf_num_cards is not None:
        # Full-model HBF jobs enter ASTRA through the hbf-background-v1
        # interactive DAG protocol. This entry enables its shared-resource
        # backend and records the independently addressable cards.
        memory_config["hbf_mem"] = {
            "memory-type": "MEMORY_POOL",
            "num-devices": hbf_num_cards,
        }
    ...
    link_bw_unit = cluster_config.get("link_bw_unit")
    astra_link_bw = _link_bw_for_astra(link_bw, link_bw_unit)
    ...
    pd_endpoint_aliases = _create_network_config(
        network_config_path, total_instances, link_bw, link_latency,
        link_bw_unit=link_bw_unit)
    ...
    cluster = {
        ...
        "link_bw": link_bw,
        # Keep the physical declaration separate from the ASTRA config value
        # so reports can audit the unit conversion without reverse-engineering
        # the generated YAML.
        "link_bw_unit": link_bw_unit,
        "link_bw_unit_effective": (
            link_bw_unit if link_bw_unit is not None else "astra_GBps_legacy"),
        "astra_link_bw": astra_link_bw,
        "link_latency": link_latency,
        "inputs_root": inputs_root,
        "network_config_path": network_config_path,
        "system_config_path": system_config_path,
        "memory_config_path": memory_config_path,
        "pd_endpoint_aliases": pd_endpoint_aliases,
    }
...
def _create_network_config(
        network_config_path, instances, link_bw, link_latency,
        link_bw_unit=None):
    ...
    num_dims = len(dims)
    endpoint_aliases, mapped_aliases, unresolved_prefills = (
        _compute_pd_endpoint_aliases(instances)
    )
    topology_rank_count = math.prod(dims)
    if len(endpoint_aliases) != topology_rank_count:
        raise ValueError(
            "Resolved instance ranks do not match the ASTRA-Sim topology: "
            f"rank_count={len(endpoint_aliases)}, "
            f"topology_rank_count={topology_rank_count}. P/D virtual "
            "receiver ranks cannot be combined with this DP topology."
        )
    topology_data = {
        "topology": FlowStyleList(["FullyConnected"] * num_dims),
        "npus_count": FlowStyleList(dims),
        "bandwidth": _normalize_network_dim_values(
            _link_bw_for_astra(link_bw, link_bw_unit), num_dims, "link_bw"),
        "latency": _normalize_network_dim_values(link_latency, num_dims, "link_latency"),
        # Keep virtual ranks as communication callback identities while
        # charging their traffic to the physical decode-side endpoint.
        "endpoint_aliases": FlowStyleList(endpoint_aliases),
    }
    ...
    if unresolved_prefills:
        logger.warning(
            "P/D network endpoint contention remains on virtual receiver "
            "rank(s) for prefill instance(s) %s because the decode peer is "
            "not unique and is selected after prefill completion.",
            unresolved_prefills,
        )

    return {
        "logical_to_physical": mapped_aliases,
        "unresolved_prefill_instances": unresolved_prefills,
        "contention_mode": (
            "decode-endpoint-alias"
            if mapped_aliases else "virtual-receiver"
        ),
    }
