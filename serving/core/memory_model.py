...
class MemoryModel():
    def __init__(self, model, instance_id, node_id, num_npus, tp_size, npu_mem, cpu_mem, block_size, fp, enable_prefix_caching, enable_prefix_sharing, prefix_pool, prefix_storage, cxl_mem=0, ep_size=1, pp_size=1, kv_cache_dtype='auto', npu_runtime_reserve_bytes=0):
        self.model = model
        ...
        self.npu_physical_mem = int(round(npu_mem * GB_TO_BYTE)) # GiB -> Byte
        if (isinstance(npu_runtime_reserve_bytes, bool)
                or not isinstance(npu_runtime_reserve_bytes, int)):
            raise TypeError(
                "npu_runtime_reserve_bytes must be an integer byte count, "
                f"got {npu_runtime_reserve_bytes!r}")
        if npu_runtime_reserve_bytes < 0:
            raise ValueError(
                "npu_runtime_reserve_bytes must be non-negative, "
                f"got {npu_runtime_reserve_bytes}")
        if npu_runtime_reserve_bytes >= self.npu_physical_mem:
            raise ValueError(
                "npu_runtime_reserve_bytes must be smaller than physical "
                f"NPU memory ({self.npu_physical_mem} bytes), got "
                f"{npu_runtime_reserve_bytes}")
        self.npu_runtime_reserve_bytes = npu_runtime_reserve_bytes
        self.npu_allocatable_mem = (
            self.npu_physical_mem - self.npu_runtime_reserve_bytes)
        # Backward-compatible capacity surface used by the scheduler and the
        # cold-KV manager. It intentionally excludes non-KV runtime/static
        # allocations, while npu_physical_mem retains the device total.
        self.npu_mem = self.npu_allocatable_mem
        self.cpu_mem = int(round(cpu_mem * GB_TO_BYTE)) # GiB -> Byte
        self.cxl_mem = int(round(cxl_mem * GB_TO_BYTE))
        ...
        # vLLM replicates KV heads when TP exceeds the model's KV-head count;
        # every rank must own at least one head.  The old logical-size/num_npus
        # formula admitted fractional KV heads and undercounted TP8 storage for
        # models such as Qwen3-30B-A3B (4 KV heads) by 2x.
        self.kv_heads_per_tp_rank = max(1, (self.kv_head + self.tp_size - 1) // self.tp_size)
        self.layers_per_pp_rank = (
            self.n_layer + self.pp_size - 1
        ) // self.pp_size
        ...
        self.npu_peak_used = self.npu_used
        self.cpu_used = 0
        if self.weight > self.npu_allocatable_mem:
            raise RuntimeError(
                f"[MemoryModel] [node={self.node_id},inst={self.instance_id}]: "
                f"Model size {self.weight * self.num_npus // GB_TO_BYTE}GB "
                "exceeds allocatable NPU memory "
                f"{self.npu_allocatable_mem * self.num_npus // GB_TO_BYTE}GB "
                f"(physical={self.npu_physical_mem * self.num_npus // GB_TO_BYTE}GB, "
                f"runtime_reserve={self.npu_runtime_reserve_bytes * self.num_npus // GB_TO_BYTE}GB)")

        if enable_prefix_caching:
            one_token_kv_size = self.get_kv(1)
            self.mem_for_kv = self.npu_allocatable_mem - self.weight
    ...
    def get_block_kv(self, batch_req, batch_len, scheduled_tokens=None):
        ...
        for i in range(batch_len):
            ...
            if req.evict or req.is_prefill():
                ...
                # Strict P/D admission may physically preallocate the whole
                # prompt on P before scheduler visibility. Treat those blocks
                # as already present while the prefill kernels materialize
                # their contents; otherwise every chunk would allocate them a
                # second time and restored prefixes could deadlock on their
                # own missing suffix capacity.
                preallocated_bytes = int(
                    req.pd_prefill_preallocated_per_rank_bytes)
                block_bytes = self.get_kv(self.block_size)
                if preallocated_bytes:
                    if block_bytes <= 0 or preallocated_bytes % block_bytes:
                        raise RuntimeError(
                            "P/D prefill preallocation is not block-aligned: "
                            f"bytes={preallocated_bytes}, "
                            f"block_bytes={block_bytes}")
                    preallocated_blocks = preallocated_bytes // block_bytes
                else:
                    preallocated_blocks = 0
                already_allocated_blocks = max(
                    blocks_before, preallocated_blocks)
                new_blocks = max(0, blocks_after - already_allocated_blocks)
    ...
    def is_free(self):
        ...
        if not is_free:
            self.logger.error(
                "Memory leak detected [node=%d, inst=%d]: "
                "NPU used: %.2fMB, CPU used: %.2fMB",
                self.node_id, self.instance_id,
                self.npu_used / MB_TO_BYTE,
    ...
    def allocate(self, size, device):
        if device == Device.NPU:
            if self.npu_used + size > self.npu_allocatable_mem:
                raise RuntimeError(
                    f"[MemoryModel] [node_id={self.node_id},inst={self.instance_id}] NPU: tried to load {size / MB_TO_BYTE:.2f}MB but only {(self.npu_allocatable_mem - self.npu_used) / MB_TO_BYTE:.2f}MB is available."
                )
            self.logger.info(
            ...
            self.npu_used += size
            self.npu_peak_used = max(self.npu_peak_used, self.npu_used)
    ...
    def is_avail(self, size, device):
        if device == Device.NPU:
            if self.npu_allocatable_mem - self.npu_used >= size:
                return True
            else:
                return False 
        elif device == Device.CPU:
    ...
    def need_size(self, size, device):
        if device == Device.NPU:
            needed = (size - (self.npu_allocatable_mem - self.npu_used))
            if needed > 0:
                return needed
            else:
                return 0
...
def full_cluster_kv_bytes_per_token(
        model, fp, kv_cache_dtype='auto', tp_size=1):
    """Bytes of KV cache per token aggregated over the full TP cluster.

    Mirrors ``MemoryModel.get_kv(1) * tp_size``.  When TP exceeds the number
    of KV heads, each rank still owns at least one head, so the physical
    cluster footprint can exceed the model's logical KV size. ``fp`` is the
    model weight dtype in bits (16, 32, ...). ``kv_cache_dtype='fp8'`` forces
    1 byte per KV element regardless of weight dtype.
    """
    ...
    # 2 (K + V) * physical KV heads * head_dim * layers * bytes_per_elem
    kv_heads_per_rank = max(1, (kv_head + tp_size - 1) // tp_size)
    return 2 * kv_heads_per_rank * tp_size * head_dim * n_layer * kv_fp
