...
class TraceCtx:
    ...
    latency_provider: object  # online analytical COMP provider, or None
...
class BatchCtx:
    ...
    online_shape: object  # OnlineBatchShape used by analytical providers
...
def _build_trace_ctx(hardware, model, config, tp_size, pp_size, local_ep, ep_total, node_id, fp,
                     ...
                     latency_model=None, latency_model_band='central',
                     dtype=None, inputs_root=None):
    ...
    latency_provider = resolve_online_latency_model(
        name=latency_model,
        repo_root=repo_root,
        hardware=hardware,
        model=model,
        ...
        band=latency_model_band,
    )
    if latency_provider is None:
        tp_needed = {max(int(tp_size), 1)}
        perf_db = _load_perf_db(
            hardware, model, variant, tp_needed, model_type)
        warn_if_runtime_exceeds_profiled(
            perf_db, runtime_max_num_batched_tokens,
            runtime_max_num_seqs)
    else:
        if gate is None or gate.routing_policy != "BALANCED":
            raise ValueError(
                "The H100/Qwen online analytical provider requires the "
                "deterministic BALANCED MoE routing policy"
            )
        perf_db = {
            "meta": {
                "online_latency_model": latency_provider.metadata(),
            },
            "architecture": _load_architecture(model_type),
            "variant": variant,
            "hardware": hardware,
            "model": model,
            "available_tps": [tp_size],
            "tables": {tp_size: {}},
        }
        if inputs_root is not None:
            provenance_path = (
                Path(inputs_root) / "trace" / hardware / model
                / "online_latency_provenance.json"
            )
            write_online_latency_provenance(
                provenance_path, latency_provider)
...
def _build_batch_ctx(batch, ctx):
    ...
    recompute_q = []
    if ctx.latency_provider is None:
        recompute_q = [0 for _ in batch.prefill_q_list]
    else:
        prefill_requests = [
            req for req in batch.requests if req.is_prefill()
        ]
        if len(prefill_requests) != len(batch.prefill_q_list):
            raise RuntimeError(
                "Batch prefill request/list mismatch: "
                f"requests={len(prefill_requests)}, "
                f"q_list={len(batch.prefill_q_list)}"
            )
        for req, q_tokens, prefix_tokens in zip(
                prefill_requests, batch.prefill_q_list,
                batch.prefill_k_list):
            chunk_start = int(prefix_tokens)
            chunk_end = chunk_start + int(q_tokens)
            intervals = []
            if req.recompute_target_tokens is not None:
                intervals.append((
                    0, int(req.recompute_target_tokens)))
            ...
            agentic_start = int(req.agentic_kv_hit_tokens)
            agentic_end = (
                agentic_start
                + int(req.agentic_kv_recompute_tokens)
            )
            if agentic_end > agentic_start:
                intervals.append((agentic_start, agentic_end))
            clipped = sorted(
                (max(chunk_start, start), min(chunk_end, end))
                for start, end in intervals
                if min(chunk_end, end) > max(chunk_start, start)
            )
            covered = 0
            covered_end = chunk_start
            for start, end in clipped:
                if end <= covered_end:
                    continue
                covered += end - max(start, covered_end)
                covered_end = end
            recompute_q.append(covered)
    online_shape = OnlineBatchShape(
        total_tokens=int(total_len),
        prefill_q=tuple(int(value) for value in batch.prefill_q_list),
        prefill_k=tuple(int(value) for value in batch.prefill_k_list),
        decode_k=tuple(int(value) for value in batch.decode_k_list),
        lm_head_sequences=int(lm_head_len),
        recompute_q=tuple(recompute_q),
    )
    if ctx.latency_provider is not None:
        ctx.latency_provider.validate_shape(online_shape)

    return BatchCtx(batch, total_len, prefill_chunk, kv_prefill, n_decode,
                    kv_decode_mean, kv_decode_max, kv_decode_min,
                    lm_head_len, decode_lens, channel_split, online_shape)
...
def _emit_layer(ctx, bctx, layer_name, lines, power_acc, batch_tag='NONE', layer_num=None,
    ...
    if ctx.latency_provider is not None:
        if category == "attention":
            latency_ns = ctx.latency_provider.attention_latency_ns(
                bctx.online_shape)
        else:
            latency_ns = ctx.latency_provider.dense_latency_ns(
                layer_name, bctx.total_len, bctx.lm_head_len)
    ...
    if power_acc is not None:
        ...
        if comm_size > 0 and comm_type != "NONE":
...
def _pd_kv_handoff_bytes(ctx, bctx):
    """Return per-rank K+V bytes produced by one prefill layer.

    The fused qkv projection output also contains Q and must not be used as
    the P/D handoff size. Match MemoryModel's physical TP layout, including
    one replicated KV head per rank when TP exceeds the number of KV heads.
    A lower-tier prefix restored only on P augments the first eligible graph;
    an HBM-retained D prefix leaves this value suffix-only.
    """
    batch = getattr(bctx, "batch", bctx)
    restored_prefix_tokens = int(getattr(
        batch, "pd_restored_prefix_handoff_tokens", 0))
    new_tokens = int(bctx.total_len)
    if restored_prefix_tokens < 0 or new_tokens < 0:
        raise RuntimeError(
            "P/D KV handoff token counts must be non-negative: "
            f"restored={restored_prefix_tokens}, new={new_tokens}")
    handoff_tokens = restored_prefix_tokens + new_tokens
    kv_heads_per_rank = max(
        1, (ctx.kv_head + ctx.tp_size - 1) // ctx.tp_size)
    return (
        2 * kv_heads_per_rank * ctx.head_dim
        * handoff_tokens * ctx.kv_fp
    )
...
def _emit_moe_block(ctx, bctx, lines, power_acc, layer_num, batch_id_str, batch_tag='NONE'):
    ...
    for i in range(emit_ep):
        ...
        if local_tokens > 0:
            if ctx.latency_provider is not None:
                rank_latency_ns = ctx.latency_provider.moe_rank_latency_ns(
                    total_tokens=effective_total_len_compute,
                    local_tokens=local_tokens,
                    activated_experts=max(activated_experts, 1),
                    ep_total=ep_total,
                )
            else:
                rank_latency_ns = _lookup_moe(
                    ctx.perf_db, local_tokens,
...
def _emit_sequence(ctx, bctx, layer_num, layers, lines, power_acc, batch_tag):
    ...
    for layer_name in layers:
        ...
        if layer_name in _TP_ALLREDUCE_AFTER:
            ...
            _emit_layer(
                ctx, bctx, layer_name, lines, power_acc, batch_tag, layer_num,
                comm_size=_pd_kv_handoff_bytes(ctx, bctx),
            )
        else:
            _emit_layer(ctx, bctx, layer_name, lines, power_acc, batch_tag, layer_num)
...
def _layer_latency_for_power(ctx, bctx, layer_name):
    ...
    if ctx.latency_provider is not None:
        if category == "attention":
            return ctx.latency_provider.attention_latency_ns(
                bctx.online_shape)
        return ctx.latency_provider.dense_latency_ns(
            layer_name, bctx.total_len, bctx.lm_head_len)
    if category == "per_sequence":
...
def _emit_pp_pd_power(ctx, bctx):
    ...
    if ctx.pd_type == 'prefill':
        # The converter sends one per-rank K+V tensor after qkv_proj in every
        # transformer layer. Reuse the exact wire-size helper so power cannot
        # silently diverge from the generated graph, then aggregate TP ranks
        # and layers. Only the final pipeline stage sends sampler output.
        kv_comm = (
            _pd_kv_handoff_bytes(ctx, bctx)
            * ctx.tp_size
        ...
        out_size = 0
        if head_layers:
            _, _, output_per_rank = calculate_sizes(
                ctx.model, head_layers[-1], bctx.lm_head_len,
                parallel=ctx.tp_size, fp=ctx.fp)
            out_size = output_per_rank * ctx.tp_size
...
def _synthesize_trace(hardware, model, config, tp_size, pp_size, local_ep, ep_total, pd_type, node_id, instance_id,
    ...
    ctx = _build_trace_ctx(hardware, model, config, tp_size, pp_size, local_ep, ep_total, node_id, fp,
                           ...
                           latency_model_band=latency_model_band,
                           dtype=dtype, inputs_root=inputs_root)
    ...
    if ctx.latency_provider is None:
        return None
    attribution = dict(
        ctx.latency_provider.recompute_attribution(bctx.online_shape))
    attribution.update({
        "latency_model": latency_model,
        "long_context_experiment": (
            ctx.latency_provider.long_context_experiment_metadata()
        ),
...
def _synthesize_interleaved_trace(hardware, model, config, tp_size, pp_size, local_ep, ep_total, pd_type, node_id, instance_id,
                                  ...
                                  tp_dim=None, ep_dim=None, dp_sum_total_len=0,
                                  latency_model=None,
                                  latency_model_band='central', dtype=None,
                                  inputs_root=None):
    ctx = _build_trace_ctx(hardware, model, config, tp_size, pp_size, local_ep, ep_total, node_id, fp,
                           placement, gate, enable_attn_offloading, power_model, pim_model, pd_type,
                           variant=variant, kv_cache_dtype=kv_cache_dtype,
...
def generate_trace(batch, hardware, tp_size, pp_size, local_ep, ep_total, pd_type=None, node_id=0, instance_id=0,
                   ...
                   tp_dim=None, ep_dim=None, dp_sum_total_len=0,
                   enable_block_copy=True, inputs_root=None,
                   latency_model=None, latency_model_band='central'):

    if (latency_model not in {None, "", "profile"}
            and enable_sub_batch_interleaving):
        raise ValueError(
            "Online analytical latency providers do not support sub-batch "
            "interleaving; disable it or use a measured profile bundle")
    ...
    if not enable_sub_batch_interleaving:
        compute_attribution = _synthesize_trace(
            *synth_args, batch, max_len, output_path, **synth_kwargs)
    else:
    ...
    if compute_attribution is not None:
        emitted_comp_node_sum_ns = sum(
            int(fields[1])
            for fields in dic
            if (len(fields) >= 11
                and fields[0] not in {"EXPERT", "PIM"})
        )
        expected_comp_node_sum_ns = int(
            compute_attribution["modeled_comp_node_sum_ns"])
        if emitted_comp_node_sum_ns != expected_comp_node_sum_ns:
            raise RuntimeError(
                "Online latency provider COMP accounting diverged from "
                "the emitted trace: "
                f"emitted={emitted_comp_node_sum_ns}, "
                f"provider={expected_comp_node_sum_ns}, "
                f"batch={batch.batch_id}"
            )
        model_compute_ns = int(
            compute_attribution["modeled_comp_critical_path_ns"])
        recompute_model_compute_ns = int(
            compute_attribution[
                "recompute_marginal_comp_critical_path_ns"])
        if not 0 <= recompute_model_compute_ns <= model_compute_ns:
            raise RuntimeError(
                "Online recompute critical-path attribution is outside its "
                "batch total: "
                f"recompute={recompute_model_compute_ns}, "
                f"total={model_compute_ns}"
            )
        batch.model_compute_ns = model_compute_ns
        batch.recompute_model_compute_ns = recompute_model_compute_ns
        batch.online_long_context_experiment = compute_attribution[
            "long_context_experiment"
        ]
        compute_attribution["emitted_comp_node_sum_ns"] = (
...
def _make_sub_batch(batch):
    ...
    for i, sub_reqs in enumerate(req_groups):
        ...
        request_ids = {req.id for req in sub_reqs}
        sub.scheduled_tokens = {
            request_id: tokens
            for request_id, tokens in sched.items()
            if request_id in request_ids
        }
        sub.pd_restored_prefix_handoff_by_request = {
            request_id: tokens
            for request_id, tokens in
            batch.pd_restored_prefix_handoff_by_request.items()
            if request_id in request_ids
        }
        sub.pd_restored_prefix_handoff_tokens = sum(
            sub.pd_restored_prefix_handoff_by_request.values())
        sub.pd_new_kv_handoff_by_request = {
            request_id: tokens
            for request_id, tokens in
            batch.pd_new_kv_handoff_by_request.items()
            if request_id in request_ids
        }
        sub.pd_new_kv_handoff_tokens = sum(
            sub.pd_new_kv_handoff_by_request.values())
