...
Without agentic tiering, the request then enters the chosen P
`Scheduler`'s waiting queue. With agentic tiering, strict P/D admission first:

1. binds the unique same-node, layout-compatible D target;
2. waits in a per-pair FIFO until an earlier preparation/P→D handoff on that
   fixed pair has cleared, then reaches a safe P/D preparation boundary;
3. claims and physically allocates the missing block-rounded full-prompt HBM
   on both P and D;
4. copies an HBM-resident reusable prefix D→P, or restores CPU/SSD state
   directly to P while reserving the complete D receive buffer; and
5. exposes the request to the P scheduler only after restore, P admission, and
   D admission have all completed.

This full P allocation prevents restored-prefix deadlock. The D allocation
prevents an unmodeled receive staging buffer or post-send capacity failure.
The pair FIFO and preparation-boundary waits are admission delays, not swap
service. A direct HBM D→P restore uses the shared congestion-aware ASTRA
topology and gates only its owner; unrelated graphs may dispatch and contend.
CPU/SSD transfer stages remain on the analytical PCIe/DRAM/SSD calendars.
...
modeled as inter-link bandwidth based on KV size. Under strict agentic P/D,
the handoff validates and consumes the already allocated D receive space; it
does not allocate the same KV bytes again.
...
2. Copies call *N*'s outgoing `inter_turn_gap_type`, `tool_wait_source`, and
   duration to call *N+1*'s incoming `return_gap_type`, `return_gap_source`,
   and `return_gap_ns`. Human, tool, mixed, and unknown remain distinct.
3. Inserts it into `_pending_requests` (sorted by arrival).
4. Goes back to **Stage 2** for that sub-request.

`Router.has_deferred_sessions()` keeps the main loop from exiting
early while sessions are still in flight.

In closed-backlog mode, the logical session keeps its active-session slot
through every non-final completion, the ensuing human/tool gap, prefill, and
P→D handoff. Templates beyond `K` stay in a FIFO admission backlog; no logical
session is dropped to make room. The slot is released only when the final
request completes on its decode owner (or colocated owner without P/D). For a
final `output_toks=1` request, the sole token is produced by prefill and the
request completes after the P→D ownership handoff without a D-side model
graph. A tier-manager `drop` event is different: it discards only a
`kv_cache_entry`, forces a later recomputation path, and neither ends the
logical session nor releases its slot.
